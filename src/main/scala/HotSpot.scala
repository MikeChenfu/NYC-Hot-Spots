...
/*********************** 200m * 200m , 2 hours adjacent neighbors in 3 space *************************/
	/*
		val lat_s=0.0025  	//200m latitude
	        val long_s=0.0025 	//200m longitude
	    	val hour_s=2*60*60 	//2 Hours=7200 seconds
		...
		//calculate the range of neighbors 
		if(degree <= lat_s)
		{
			ratio=(lat_s/degree).toInt
			wind_x=(-ratio to ratio)
	                wind_y=(-ratio to ratio)
	
		}
		if(hour <= hour_s )
		{
			ratio=(hour_s/hour).toInt
			wind_z=(-ratio to ratio)
		}
	*/

/************************ 26 topologically adjacent neighbors in 3 space ****************************/
	...
	def main(args: Array[String]) {
		...
		//Index generation : (cell_x ID, cell_y ID, time_step ID, number of passangers )
    		val  IndexG=filterData.map(e => {
			 ...
			 //e: (time,(longitude,latitude),number of passangers ) --> (x,y,z)
			 val(x,y,z)=e
		...
		//IndexG.collect().foreach(println)  //debug Index
		...
		//println("Savg:"+Savg+" Xavg:"+Xavg); //debug Xavg and Savg
